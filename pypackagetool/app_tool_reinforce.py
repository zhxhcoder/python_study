...
def upload_file(url, apkpath):
	params = { 'username' : reinforce_username, 'password' : reinforce_password, 'apk' : open(apkpath, 'rb'), 'tactics_id' : reinforce_tactics_id }
...
def download_apk(url, upload_result, apkpath):
	if_download = True
	while if_download:
		params = { 'username' : reinforce_username, 'password' : reinforce_password, 'apkinfo_id' : str(upload_result['apkinfo']['apk_id']) }
